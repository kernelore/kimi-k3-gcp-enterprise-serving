...
# Prefill backend for the 24 full-attention (MLA) layers. SGLANG_ATTENTION_BACKEND is
# accepted as a deprecated alias: the old name emitted --attention-backend, which set the
# base for BOTH phases and therefore silently pinned decode as well.
export SGLANG_PREFILL_ATTENTION_BACKEND="${SGLANG_PREFILL_ATTENTION_BACKEND:-${SGLANG_ATTENTION_BACKEND:-triton}}"
# Empty by default: SGLang resolves MLA-on-Blackwell to flashinfer on its own. Pin only
# with measurements in hand.
export SGLANG_DECODE_ATTENTION_BACKEND="${SGLANG_DECODE_ATTENTION_BACKEND:-}"
# KDA linear-attention prefill kernel for the 69 KDA layers.
export SGLANG_LINEAR_ATTN_PREFILL_BACKEND="${SGLANG_LINEAR_ATTN_PREFILL_BACKEND:-cutedsl}"

...
if [ -n "${AUTH_CIDR}" ]; then
  ...
  # No spaces inside the brackets: `terraform fmt` rewrites `[ { ... } ]` to `[{ ... }]`,
  # and T1_F3_01 runs `fmt -check` over the whole terraform/ directory -- including this
  # generated file -- so the emitted form has to be the canonical one or the gate fails
  # on every machine that has actually run this script.
...
# The GPU pool has to be able to hold every serving pod, and podAntiAffinity puts one pod
# per node, so the floor is SERVING_REPLICAS x NODES_PER_REPLICA. Deriving it here rather
# than making the operator keep two numbers in step means SERVING_REPLICAS=2 is sufficient
# on its own; an explicit GPU_POOL_MAX_NODES/GPU_MAX_NODES above the floor still wins,
# since the fabric gates and the sweep sometimes want headroom.
SERVING_NODE_FLOOR=$(( ${SERVING_REPLICAS:-1} * ${NODES_PER_REPLICA:-2} ))
TF_GPU_POOL_MAX_NODES="${GPU_POOL_MAX_NODES:-${GPU_MAX_NODES:-2}}"
if [ "${TF_GPU_POOL_MAX_NODES}" -lt "${SERVING_NODE_FLOOR}" ]; then
  echo "    [NOTE] Raising gpu_pool_max_nodes ${TF_GPU_POOL_MAX_NODES} -> ${SERVING_NODE_FLOOR}"
  echo "           (SERVING_REPLICAS=${SERVING_REPLICAS:-1} x NODES_PER_REPLICA=${NODES_PER_REPLICA:-2});"
  echo "           a pool capped below the pod count leaves the surplus pods Pending, not absent."
  TF_GPU_POOL_MAX_NODES="${SERVING_NODE_FLOOR}"
fi
...
gpu_pool_max_nodes            = ${TF_GPU_POOL_MAX_NODES}

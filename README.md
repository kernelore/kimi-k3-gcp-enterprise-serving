...
│   ├── 08_run_measurement_window.sh # Supervised spend window: owns the cluster, arms a teardown watchdog, resumes per stage
│   ├── lib/serving_render_defaults.sh # Shared render defaults + dry-run validation for every path that renders the StatefulSet

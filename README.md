...
1.  **SGLang (Primary Default | `INFERENCE_ENGINE="sglang"`)**:
    ...
    -   **Performance Optimizations**: Utilizes `--moe-runner-backend flashinfer_mxfp4 --kv-cache-dtype fp8_e4m3` with RadixAttention prefix caching, and sizes the KDA recurrent-state pool with `--mamba-full-memory-ratio 7.21`. Note that prefix caching reuse benefits only the 24 MLA attention layers; the 69 KDA linear recurrent state layers are not prefix-shareable.
    -   **Hybrid Attention Backends**: K3 interleaves 69 KDA linear-attention layers with 24 full-attention (MLA) layers, so each phase is configured independently rather than through the combined `--attention-backend` flag, which would set the base for both at once. `SGLANG_PREFILL_ATTENTION_BACKEND` (default `triton`) emits `--prefill-attention-backend` for the MLA layers, and `SGLANG_LINEAR_ATTN_PREFILL_BACKEND` (default `cutedsl`) emits `--linear-attn-prefill-backend` for the Blackwell KDA CuteDSL kernel — a choice SGLang admits only for this MLA-hybrid family. `SGLANG_DECODE_ATTENTION_BACKEND` is **empty by default** so the engine's own SM100 dispatch applies: SGLang classifies `KimiK3ForConditionalGeneration` as `AttentionArch.MLA` and resolves MLA-on-Blackwell to FlashInfer. Pinning `flashmla` (a Hopper kernel) or `trtllm_mha` (the SM100 default for MHA architectures, which K3 is not) would both be wrong here; `tests/check_render_exceptions.sh` fails the render if either reappears.
...
**Weight & Kernel Format**    | Native `KimiK3ForConditionalGeneration` / MXFP4 / FlashInfer + KDA CuteDSL | Direct PyTorch checkpoint loading (Experimental)

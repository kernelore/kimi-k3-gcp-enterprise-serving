...
def generate_markdown(sglang: dict | None, trtllm: dict | None) -> str:
    if sglang is not None and trtllm is not None:
        ...
        if best_cell is not None:
            ...
            lines.append("")
        lines.append(f"**Method.** Suites never overlap — each is its own Kubernetes Job, started only once the previous has drained. Every prompt carries a 16-character random nonce, so no two requests share a radix-cache prefix (0% prefix-cache hits by construction, not by a cache-flush API). Engine identity is read from the running pod, not from benchmark config, and stamped into every result file; the provenance gate here and `tests/adv_audit_benchmark_integrity.py` reject a set whose suites overlap, run out of order, or carry a placeholder version. {other_eng_name} was not benchmarked here, so delta columns are omitted.")
        lines.append("")

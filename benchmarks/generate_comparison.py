...
def validate_provenance(sglang: dict, trtllm: dict):
    """
    ...
    for s in suites:
        ...
        t_eng = t_meta.get("engine", "")
        t_img = t_meta.get("image", "")
...
def validate_parity_and_sanity(sglang: dict, trtllm: dict):
    suites = ["standard", "massive", "soak", "saturation", "prefill"]
    for s in suites:
        g = sglang[s]
        t = trtllm[s]
        ...
        # 2. Sanity Gates
        for eng_name, data in [("SGLang", g), ("TRT-LLM", t)]:
...
def main():
    ...
    print("Loading dual-engine benchmark JSON results...")
    for s in suites:
    ...
    try:
        ...
        validate_parity_and_sanity(sglang_data, trtllm_data)
